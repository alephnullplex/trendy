...
class ServicesBaseTestCases(TestCase):
    ...
    def test_default_calls_transport(self):
        self.trend.client.factory.create = MagicMock(return_value=True)
        sut = ServiceBase(self.trend, 'test')
        sut.default()

        self.trend.client.factory.create.assert_called_once_with('testTransport')
